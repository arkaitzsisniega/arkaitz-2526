...
def _palabra_aparece(texto_sin_tildes: str, palabra: str) -> bool:
    """Word-boundary relajada: la palabra aparece rodeada por no-alfanuméricos."""
    import re as _re
...
def _detectar_intent_carga_ultima(prompt: str) -> Optional[str]:
    """Detecta preguntas tipo 'carga jugador por jugador', 'borg del entreno
    de ayer', 'qué tal el entrenamiento de ayer? dime las cargas',
    'cargas del entrenamiento'.

    Devuelve fecha YYYY-MM-DD si especifica una, '' si es la última, None
    si no matchea. Tolerante a entreno/entrenamiento, carga/cargas,
    sesión/sesiones, con o sin tildes.
    """
    if not prompt:
        return None
    p = prompt.lower()
    ...
    SUSTANTIVOS_METRICA = ("carga", "cargas", "borg")
    CONTEXTO_TEMPORAL = (
        "ayer", "hoy", "anteayer", "ultim", " de ", " del ",
    )
